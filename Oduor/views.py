...
def home(request):
    ...
    if selected_categories:
        Details = Details.filter(categories__name__in=selected_categories)
    ...
    # fetch both stuffs and categories
...
def import_from_excel(request):
    if request.method == 'POST':
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            stuffs.objects.create(
                name_of_tool=name,
                description=description,
                tutorial_tool=sign_up,
                ad=ad,
                youtube_link=video,
                website_link=web_link
            )

        # when inside the for loop, it only returns one row.
